...
A thorough exploration of the `blueprint-press-dryrun` repository produced the
...
- **Package**: `src/blueprint_press_dryrun/` (PEP 420 layout with uv)
- **CLI**: `bpd` (Click-based, example in `src/blueprint_press_dryrun/cli/`)
...
[[replace]]
field = "package_name"
current = ["blueprint_press_dryrun"]
files = ["pyproject.toml", "src/blueprint_press_dryrun/__init__.py", ...]
...
[[rename]]
from = "src/blueprint_press_dryrun/"
to   = "src/{package_name}/"
...
4. **Apply** (remove → replace → rename):
   ...
   - **Rename**: Move files/dirs (e.g., `src/blueprint_press_dryrun/` → `src/{new_name}/`)
...
| 4 | **Fork** | `<user>/blueprint-press-dryrun` (name collides, owner differs) | Full blueprint history | Guard's skip-on-origin checks owner + name (must NOT skip forks; contributor sentinel is escape hatch) | L1 + L2 + contrib sentinel test |
...
```
              ┌──────────────────────────────────────────────┐
              │       blueprint-press-dryrun (a working repo)   │
              └──────────────────────────────────────────────┘
                     │                            │
...
A) WORKING REPO + MANIFEST (current)   B) PLACEHOLDER TEMPLATE (cookiecutter/copier)
   ...
   repo contains real strings:            repo contains jinja:
     "blueprint_press_dryrun"                  "{{ package_name }}"

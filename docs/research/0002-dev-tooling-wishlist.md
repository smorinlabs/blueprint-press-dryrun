...
| WL-025 | Documented zero-install run path | First-class `uv tool install` / `uvx bpd` and pipx docs in README, verified by the WL-004 smoke test. (Standalone PyInstaller binaries: defer — maintenance-heavy.) | Users try the CLI in one command; lowers adoption friction for the template's forks too. | CLI-specific, low effort |
...
| WL-004 | `build-smoke` job, uv end-to-end: `uv build` → `twine` metadata check → install wheel *and* sdist into fresh envs → `bpd --version` |
...
| WL-023 | Syrupy golden snapshots of all `bpd` commands' `--help`, width pinned to 80 for determinism |

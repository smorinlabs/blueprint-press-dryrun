...
uv run python -c "import blueprint_press_dryrun; print(blueprint_press_dryrun.__version__)"
...
- **Build cannot find the package module**: check `[tool.uv.build-backend]` and keep `module-name = "blueprint_press_dryrun"` with `module-root = ""`.

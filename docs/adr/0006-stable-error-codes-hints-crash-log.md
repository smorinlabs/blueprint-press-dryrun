...
1. Every expected error carries a **stable `BPD###` error code** (a string
...
2. **Unexpected exceptions** — including failures while building the
   `AppContext` at startup — emit `BPD000`, exit `IO` (4), and always
   append the full traceback to `<state>/bpd/bpd_crash.log`, printing
...
`just init` rewrites the prefix wholesale — same convention as the `BPD_*`
env vars.

...
- **Related:** [design 0001](../design/0001-bpd-cli-conventions.md) §8 (R8)
...
secrets live in the `--token` flag or `$BPD_TOKEN` env var only: config
...
The token resolves from `--token` then `$BPD_TOKEN` — **never** from a
...
- Setup docs must say "export BPD_TOKEN" instead of "run config set".

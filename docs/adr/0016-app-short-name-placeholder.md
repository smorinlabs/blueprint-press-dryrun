# 0016. App short name becomes an obvious placeholder: `bpd` → `acmeapp`

- **Status:** Accepted (implementation pending — the mechanical rename has not
  been applied yet; until it lands, `bpd`/`BPD` remain the live identity)
- **Date:** 2026-06-12
- **Deciders:** maintainer (interactive)
- **Supersedes:** [ADR 0001](0001-app-short-name-bpd.md) — the *choice of
  name* only; the hard-rename / no-back-compat-alias policy ADR 0001 set
  still stands
- **Related:** [design 0001](../design/0001-bpd-cli-conventions.md) §0–§1;
...
user-visible surface: it is the console command, the `BPD_*` env-var prefix,
the XDG namespace (`~/.config/<app>/`), and the file-name prefix
(`<app>_config.toml`, `<app>.log`).

`bpd` (ADR 0001) was invented for substring-disjointness — text-mode
...
- **Doc readers** can't tell whether `bpd config set token` refers to a real
...
- **AI agents** working in forks pattern-match `BPD_*` as intentional
  project vocabulary and propagate it into new code, instead of flagging it
  as a placeholder to replace.

## Decision

Replace `bpd` / `BPD` with **`acmeapp` / `ACMEAPP`** everywhere the
...
- **Breaking change** when implemented: the console command, env prefix,
  ...
  still-standing policy, this is a hard rename with no `bpd` alias.
...
- Until the rename lands, this file mentions the live `bpd` / `BPD`
  identity and is therefore tracked under `app_name` / `app_name_upper` in
  `init/manifest.toml`.

## Alternatives considered

- **Keep `bpd`, add disclaimers** (README/AGENTS notes that it's a
